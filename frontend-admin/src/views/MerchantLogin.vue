  ...
  <div class="auth-page">
    <div class="auth-card">
      ...
      <div class="auth-header">
        ...
        <h2>商家登录</h2>
        <p class="auth-subtitle">登录商家中心，管理您的店铺</p>
      </div>
      <el-form :model="form" @submit.prevent="login">
        ...
        <el-form-item>
          <button type="button" class="btn-submit" @click="login" :disabled="loading">
            {{ loading ? '登录中...' : '登 录' }}
          </button>
        </el-form-item>
      </el-form>
...
async function login() {
  ...
  loading.value = true
  try {
    const res = await request.post('/api/merchant/login', form)
...
.auth-footer { text-align: center; margin-top: 20px; font-size: 13px; color: #8C8C8C; }
